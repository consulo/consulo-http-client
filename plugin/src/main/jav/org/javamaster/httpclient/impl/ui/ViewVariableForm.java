...
public class ViewVariableForm extends DialogWrapper {
    ...
    public ViewVariableForm(Project project) {
        ...
        setModal(false);
        setResizable(false);
        setOKButtonText(NlsBundle.INSTANCE.message("close"));
        setTitle(NlsBundle.INSTANCE.message("available.variables"));
    ...
    private void initTable() {
        ...
        List<Trinity<String, Map<String, String>, Integer>> resList = collectVariableMapList();
    ...
    private List<Trinity<String, Map<String, String>, Integer>> collectVariableMapList() {
        ...
        List<Trinity<String, Map<String, String>, Integer>> resList = Lists.newArrayList();
        ...
        resList.add(new Trinity<>(NlsBundle.INSTANCE.message("file.global.desc"), fileGlobalVariables, TYPE_GLOBAL));

        Map<String, String> variableMap = variableResolver.getJsGlobalVariables();
        resList.add(new Trinity<>(NlsBundle.INSTANCE.message("js.variable.desc"), variableMap, TYPE_JS));

        Map<String, String> envMap = EnvFileService.Companion.getEnvMap(project, false);
        resList.add(new Trinity<>(NlsBundle.INSTANCE.message("environment.variable.desc"), envMap, TYPE_ENV));
        ...
        resList.add(new Trinity<>(NlsBundle.INSTANCE.message("system.desc"), propMap, TYPE_SYS));

        Map<String, String> eMap = Maps.newLinkedHashMap();
        System.getenv().forEach((key, value) -> eMap.put(ENV_PREFIX + "." + key, value));
        resList.add(new Trinity<>(NlsBundle.INSTANCE.message("env.desc"), eMap, TYPE_SYS));

        return resList;
    }

    private int calRows(List<Trinity<String, Map<String, String>, Integer>> resList) {
        int rows = 0;
        for (Trinity<String, Map<String, String>, Integer> triple : resList) {
            rows++;
    ...
    private Object[][] createRowData(List<Trinity<String, Map<String, String>, Integer>> resList, int rows) {
        JButton btnAdd = new JButton(NlsBundle.INSTANCE.message("add"));
        btnAdd.setForeground(new JBColor(new Color(64, 158, 255), new Color(64, 158, 255)));

        JButton btnEdit = new JButton(NlsBundle.INSTANCE.message("edit"));
        ...
        for (Trinity<String, Map<String, String>, Integer> triple : resList) {
            ...
            Map<String, String> map = triple.getSecond();
            if (map.isEmpty()) {
                rowData[i][0] = NlsBundle.INSTANCE.message("no.data.available");
                rowData[i][1] = "";
                rowData[i][2] = "";
                rowData[i][3] = -1;
                i++;
