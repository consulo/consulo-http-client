...
import org.javamaster.httpclient.impl.dashboard.support.JsTgz;
...
import org.javamaster.httpclient.impl.mock.MockServer;
...
import org.javamaster.httpclient.impl.ws.WsRequest;
...
public class HttpProcessHandler extends BaseProcessHandler {
    ...
    public HttpProcessHandler(HttpMethod httpMethod, String selectedEnv) {
        ...
        this.tabName = HttpUtils.getTabName(httpMethod);
        ...
        this.jsExecutor = new JsExecutor(project, httpFile, tabName);
        ...
        this.version = httpVersion != null ? httpVersion.getVersion() : HttpClient.Version.HTTP_1_1;
    }
...
//    }
    ...
    private void handleHttp(
        ...
        CompletableFuture<java.net.http.HttpResponse<byte[]>> future =
            methodType.execute(url, version, reqHeaderMap, reqBody, httpReqDescList, tabName, paramMap);
            ...
            Application.get().invokeLater(() -> {
                Application.get().runWriteAction(() -> {
                    try {
                        ...
                        String comment = NlsBundle.message("res.desc", response.statusCode(), costTimes, size);
